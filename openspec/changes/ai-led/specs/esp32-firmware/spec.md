...
#### Scenario: 有源蜂鸣器连接
- **WHEN** 连接有源蜂鸣器（低电平触发）
- **THEN** 正极(+) 接 GPIO4（左侧第4个引脚），负极(-) 接 ESP32 GND

#### Scenario: 避免引脚冲突
- **WHEN** 选择 GPIO 引脚
- **THEN** 不使用 GPIO8（板载蓝灯），GPIO0-GPIO3 保留给 ADC/SPI
...
#### Scenario: 接收状态命令
- **WHEN** 通过 BLE 写入 UTF-8 字符串命令（格式为 `mode[,buzzer_duration_ms]`）
- **THEN** ESP32 解析命令并切换对应 LED 状态，buzzer_duration_ms 参数控制蜂鸣器响的毫秒数

#### Scenario: 命令格式
- **WHEN** 接收命令字符串
- **THEN** 命令格式为 `mode[,buzzer_duration_ms]`，其中 mode 为 0-19 数字或名称，buzzer_duration_ms 为蜂鸣器响的毫秒数（0=不响，默认 0）
...
### Requirement: 蜂鸣器控制
ESP32 SHALL 根据命令中的 buzzer_duration_ms 参数控制有源蜂鸣器。

#### Scenario: 蜂鸣器响指定时长
- **WHEN** 接收命令格式为 `mode,200`（如 "alarm,200"）
- **THEN** 蜂鸣器鸣响 200ms

#### Scenario: 蜂鸣器极短提示音
- **WHEN** 接收命令格式为 `mode,20`（如 "alarm,20"）
- **THEN** 蜂鸣器鸣响 20ms（极短滴一声）
